...
export function ProductsTable({
  ...
  function openEdit(product: Product) {
    ...
    setFormSpecialName(product.specialDiscountName ?? '')
    setFormSpecialPercent(String(product.specialDiscountPercent ?? 0))
    setFormSpecialStartsAt(product.specialDiscountStartsAt ? product.specialDiscountStartsAt.slice(0, 10) : '')
    setFormSpecialEndsAt(product.specialDiscountEndsAt ? product.specialDiscountEndsAt.slice(0, 10) : '')
    setFormAgentInstructions(product.agentInstructions ?? '')
    setFormImage(null)
  ...
  function handleSave() {
    ...
    if (editingProduct) formData.append('active', String(formActive))
      ...
      {products.length === 0 ? (
        ...
        >
          <div className="relative">
            ...
            <div className="relative flex h-20 w-20 items-center justify-center rounded-2xl bg-surface-2 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
              <Package className="h-9 w-9 text-text-subtle/40" />
            </div>
          </div>
        ...
        >
          {products.map((product) => {
            const now = new Date()
            const hasSpecial =
              product.specialDiscountPercent > 0 &&
              product.specialDiscountStartsAt &&
              product.specialDiscountEndsAt &&
              now >= new Date(product.specialDiscountStartsAt) &&
              now <= new Date(product.specialDiscountEndsAt)

            const discounts: { label: string; value: string; color: string }[] = []
            if (product.pixDiscountPercent > 0)
              discounts.push({ label: 'PIX', value: formatPercent(product.pixDiscountPercent), color: 'text-emerald' })
            if (product.cardDiscountPercent > 0)
              discounts.push({ label: 'Cartao', value: formatPercent(product.cardDiscountPercent), color: 'text-teal' })
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {hasSpecial && product.active && (
                    <div className={`absolute right-0 ${product.imageUrl ? 'top-0' : 'top-[3px]'} overflow-hidden`}>
                      <div className="bg-accent px-3 py-1 text-[10px] font-bold tracking-wide text-white shadow-[0_2px_8px_rgba(212,130,10,0.3)]">
                        {product.specialDiscountName || `${formatPercent(product.specialDiscountPercent)} OFF`}
                      </div>
                    </div>
                  )}
                      ...
                      <div className="min-w-0 flex-1">
                        <h3
                          className={`font-title text-base font-semibold leading-tight ${
                            product.active ? 'text-text-primary' : 'text-text-subtle'
                          }`}
                        >
                          {product.name}
                        </h3>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-subtle">
                      {editingProduct ? 'Altere as informações do produto' : 'Preencha os dados para criar'}
                    </p>
                  </div>
                </div>
                <Dialog.Close
                    ...
                    <div className="border-t border-border-default/50 px-4 py-4">
                      <div className="space-y-1.5">
                        <label className="text-xs font-medium text-text-subtle">
                          Como o agente deve apresentar este produto?
                        </label>
                        <textarea
                          value={formAgentInstructions}
                          onChange={(e) => setFormAgentInstructions(e.target.value)}
                          placeholder="Ex: Destaque a qualidade premium e a garantia de 1 ano. Ideal para quem busca durabilidade..."
                          maxLength={2000}
                          rows={3}
                          className="w-full resize-none rounded-xl border border-border-default bg-surface-2 px-3 py-2.5 text-sm text-text-primary outline-none placeholder:text-text-subtle transition-colors focus:border-accent/40 focus:ring-1 focus:ring-accent/15"
                        />
